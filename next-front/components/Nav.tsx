...
export default function Nav() {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-48 lg:w-64 shrink-0 flex-col">
          ...
          {LINKS.map(({ href, label, Icon }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`group relative flex items-center gap-3 px-4 lg:px-6 py-4 transition-all duration-200 ${
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#14110f] border-t-2 border-stone-800 flex items-stretch">
        {LINKS.map(({ href, label, Icon }) => {
          const active = href === '/' ? pathname === '/' : pathname.startsWith(href);
          return (
            <Link
              key={href}
              href={href}
              className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 text-[10px] uppercase tracking-wider font-bold transition-colors ${
                active ? 'text-amber-400 bg-stone-900/40' : 'text-stone-600'
              }`}
            >
              <Icon size={18} />
              <span>{label}</span>
